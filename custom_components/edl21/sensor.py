...
# NOTE: The hard-coded ``MIN_TIME_BETWEEN_UPDATES = timedelta(seconds=60)``
# from upstream is replaced by ``min_time`` derived from the options
# flow (see ``async_setup_entry``). Default = ``DEFAULT_SCAN_INTERVAL``
# seconds.
...
async def async_setup_entry(
    ...
    min_time = timedelta(seconds=scan_interval)
    api = EDL21(hass, config_entry.data, async_add_entities, min_time)
    await api.connect()
...
class EDL21:
    ...
    def __init__(
        ...
        LOGGER.debug(
            "Initialized EDL21 on %s with %ds throttle",
            config[CONF_SERIAL_PORT],
            int(min_time.total_seconds()),
        )
            ...
            if (electricity_id, obis) in self._registered_obis:
                ...
                if entity_description:
                    ...
                    LOGGER.warning(
                        ...
                        obis,
                        "https://github.com/palana/edl21/issues",
                    )
